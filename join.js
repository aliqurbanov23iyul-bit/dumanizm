...
function validate(step){
 ...
 if(step===1){const v=parseInt($('#inp-age')?.value);if(!v||v<14||v>99){toast('Yashi duzgun daxil et (14-99)');return false;}formData.age=v;}
 if(step===2){const v=$('#inp-song')?.value.trim();if(!v){toast('Sevdiyin mahnini yaz');return false;}formData.favorite_song=v;}
 if(step===3){const v=$('#inp-phone')?.value.trim();if(!v||v.length<6){toast('Nomreni duzgun daxil et');return false;}formData.phone=v;}
 return true;
}
